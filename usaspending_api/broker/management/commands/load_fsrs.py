...
class Command(BaseCommand):
    ...
    def create_subaward(self, row, shared_award_mappings, award_type):
        ...
        if row["internal_id"] not in shared_award_mappings:
            ...
            if cfda:
                subaward_dict.update(
                    {"cfda_number": cfda.program_number, "cfda_title": cfda.program_title, "cfda_id": cfda.pk}
                )
...
def get_city_and_county_from_state(state, city_name):
    ref_loc = (
        RefCityCountyCode.objects.filter(state_code=state, city_name=city_name)
        .values("county_name", "county_code", "city_code")
        .first()
    )
    if ref_loc:
        return ref_loc
