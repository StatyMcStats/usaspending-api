...
class Command(BaseCommand):
    ...
    def set_lookup_maps(self):
        ...
        self.agency_toptier_map = {
            agency.toptier_agency.cgac_code: agency for agency in Agency.objects.filter(toptier_flag=True)
        }
    ...
    def diff_fpds_data(self, db_cursor, ds_cursor, fiscal_year=None):
        db_query = "SELECT detached_award_procurement_id " "FROM detached_award_procurement"
        db_arguments = []

        ds_query = "SELECT detached_award_procurement_id " "FROM transaction_fpds"
        ds_arguments = []
    ...
    def load_locations(self, fpds_broker_data, total_rows, pop_flag=False):
        ...
        for index, row in enumerate(fpds_broker_data, 1):
            if not (index % 10000):
                logger.info(
                    "Locations: Loading row {} of {} ({})".format(
            ...
            if location_country_code_obj:
                ...
                if location_country_code_obj.country_code != "USA":
                    location_value_map.update({"state_code": None, "state_name": None})
            else:
                # no country found for this code
                location_value_map.update({"location_country_code": None, "country_name": None})

            location_instance_data = load_data_into_model(
                Location(), row, value_map=location_value_map, field_map=field_map, as_dict=True
            )
    ...
    def load_legal_entity(self, fpds_broker_data, total_rows):
        ...
        for index, row in enumerate(fpds_broker_data, 1):
            if not (index % 10000):
                logger.info(
                    "Legal Entity: Loading row {} of {} ({})".format(
                        str(index), str(total_rows), datetime.now() - start_time
                    )
                )
            ...
            if not legal_entity:
                ...
                legal_entity = load_data_into_model(
                    legal_entity, row, value_map={"location": lel_bulk[index - 1]}, save=False
                )
    ...
    def load_parent_awards(self, fpds_broker_data, total_rows):
        ...
        for index, row in enumerate(fpds_broker_data, 1):
            if not (index % 10000):
                logger.info(
                    "Parent Awards: Loading row {} of {} ({})".format(
                        str(index), str(total_rows), datetime.now() - start_time
                    )
                )
    ...
    def load_awards(self, fpds_broker_data, total_rows):
        ...
        for index, row in enumerate(fpds_broker_data, 1):
            if not (index % 10000):
                logger.info(
                    "Awards: Loading row {} of {} ({})".format(str(index), str(total_rows), datetime.now() - start_time)
                )
            ...
            funding_agency = self.agency_no_sub_map.get((row["funding_agency_code"], row["funding_sub_tier_agency_co"]))
    ...
    def load_transaction_normalized(self, fpds_broker_data, total_rows):
        ...
        for index, row in enumerate(fpds_broker_data, 1):
            ...
            contract_field_map = {"type": "contract_award_type", "description": "award_description"}
    ...
    def load_transaction_fpds(self, fpds_broker_data, total_rows):
        ...
        for index, row in enumerate(fpds_broker_data, 1):
            if not (index % 10000):
                logger.info(
                    "Transaction FPDS: Loading row {} of {} ({})".format(
                        str(index), str(total_rows), datetime.now() - start_time
                    )
                )

            fpds_instance_data = load_data_into_model(TransactionFPDS(), row, as_dict=True)  # thrown away
    ...
    def handle(self, *args, **options):
        ...
        with timer("Diff-ing FPDS data", logger.info):
            to_insert, to_delete = self.diff_fpds_data(
