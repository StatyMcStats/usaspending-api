...
class Command(BaseCommand):
    help = (
        "Takes in the authoritative_agencies_list.csv and the broker_agency_list.csv to make"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            default=os.path.join(
                django.conf.settings.BASE_DIR, "usaspending_api", "data", "authoritative_agency_list.csv"
            ),
        )
        parser.add_argument(
            "--broker_agency_list",
            ...
            default=os.path.join(django.conf.settings.BASE_DIR, "usaspending_api", "data", "broker_agency_list.csv"),
        )
    ...
    def handle(self, *args, **options):
        ...
        broker_agency_list_df["CGAC.AGENCY.CODE"] = broker_agency_list_df["CGAC.AGENCY.CODE"].apply(
            lambda x: x.zfill(3)
        )

        # Remove all the rows where awarding_agency_name is NA, awarding_sub_tier_agency_n is also NA
        broker_agency_list_df = broker_agency_list_df[pd.notnull(broker_agency_list_df["awarding_agency_name"])]
        # Remove other columns and duplicates
        broker_agency_list_df = broker_agency_list_df[["CGAC.AGENCY.CODE", "SUBTIER.CODE"]].drop_duplicates()
        # keep only the rows from agency list that have a cgac and subtier combo from broker_agency_list
        modified_agency_list = pd.merge(
            agencies_list_df,
            broker_agency_list_df,
        ...
        modified_agency_list.to_csv(
            os.path.join(agencies_dir, "modified_authoritative_agency_list.csv"), mode="w", index=False
        )
