...
class Command(BaseCommand):
    ...
    help = (
        "Loads a single submission from the DATA Act broker."
...
def load_file_d1(submission_attributes, procurement_data, db_cursor, quick=False):
    ...
    for index, row in enumerate(procurement_data, 1):
        ...
        if row["funding_agency_code"] is None or len(row["funding_agency_code"].strip()) < 1:
            ...
            row["funding_agency_code"] = funding_agency.toptier_agency.cgac_code if funding_agency is not None else None

        # Find the award that this award transaction belongs to. If it doesn't exist, create it.
        awarding_agency = Agency.get_by_toptier_subtier(row["awarding_agency_code"], row["awarding_sub_tier_agency_c"])
        ...
        )  # It is a FAIN/PIID/URI, not our db's pk
        award.save()
...
def load_file_d2(
    ...
    for index, row in enumerate(award_financial_assistance_data, 1):
        if not (index % 100):
            logger.info(
                "D2 File Load: Loading row {} of {} ({})".format(
                    str(index), str(total_rows), datetime.now() - start_time
                )
            )
        ...
        awarding_agency = Agency.get_by_toptier_subtier(row["awarding_agency_code"], row["awarding_sub_tier_agency_c"])
        created, award = Award.get_or_create_summary_award(
            awarding_agency=awarding_agency, piid=row.get("piid"), fain=row.get("fain"), uri=row.get("uri")
        )
        award.save()
        ...
        financial_assistance_data = load_data_into_model(
            TransactionFABS(), row, field_map=fad_field_map, value_map=fad_value_map, as_dict=True  # thrown away
        )
