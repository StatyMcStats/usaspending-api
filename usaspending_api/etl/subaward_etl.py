...
WHERE  award_procurement.submission_id = %s AND award_procurement.piid IN %s
"""
...
def load_subawards(submission_attributes, awards_touched, db_cursor):
    ...
    for row in d1_f_data:
        ...
        award = (
            Award.objects.filter(
                ...
                latest_transaction__contract_data__parent_award_id=row["parent_award_id"],
            )
            .distinct()
        ...
        recipient, created = LegalEntity.objects.get_or_create(
            recipient_unique_id=row["duns"], recipient_name=row["company_name"]
        )
        ...
        subaward, created = Subaward.objects.update_or_create(
            subaward_number=row["subcontract_num"], award=award, defaults=d1_f_dict
        )
        if created:
            d1_create_count += 1
        else:
    ...
    for row in d2_f_data:
        ...
        if not agency:
            logger.warn(
                "Subaward number {} cannot find matching agency with toptier code {} and subtier "
                "code {}".format(row["subaward_num"], row["awarding_agency_code"], row["awarding_sub_tier_agency_c"])
            )
            continue
        ...
        if row["fain"] and len(row["fain"]) > 0:
            award = (
                Award.objects.filter(awarding_agency=agency, latest_transaction__assistance_data__fain=row["fain"])
                .distinct()
                .order_by("-date_signed")
                .first()
            )
        ...
        if not award and row["uri"] and len(row["uri"]) > 0:
            award = (
                Award.objects.filter(awarding_agency=agency, latest_transaction__assistance_data__uri=row["uri"])
                .distinct()
                .first()
            )
        ...
        if not award:
            logger.warn(
                "Subaward number {} cannot find matching award with fain {}, uri {}; "
        ...
        # Get or create unique DUNS-recipient pair
        recipient, created = LegalEntity.objects.get_or_create(
            recipient_unique_id=row["duns"], recipient_name=recipient_name
        )
        ...
        # Create the subaward
        subaward, created = Subaward.objects.update_or_create(
            subaward_number=row["subaward_num"], award=award, defaults=d2_f_dict
        )
        if created:
            d2_create_count += 1
        else:
    ...
    logger.info(
        """Submission {}
           ...
           Subawards created: {}
           Subawards updated: {}
           Empty subaward rows: {}""".format(
            submission_attributes.broker_submission_id,
            d1_create_count,
            d1_update_count,
            d1_empty_count,
            d2_create_count,
