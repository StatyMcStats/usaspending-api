...
class DownloadListAgenciesViewSet(APIDocumentationView):
    ...
    modified_agencies_list = os.path.join(
        settings.BASE_DIR, "usaspending_api", "data", "modified_authoritative_agency_list.csv"
    )
    sub_agencies_map = {}
    ...
    def post(self, request):
        ...
        toptier_agencies = list(
            ToptierAgency.objects.filter(cgac_code__in=used_cgacs).values("name", "toptier_agency_id", "cgac_code")
        )

        if not agency_id:
            # Return all the agencies if no agency id provided
            cfo_agencies = sorted(
                list(filter(lambda agency: agency["cgac_code"] in CFO_CGACS, toptier_agencies)),
            ...
            response_data["sub_agencies"] = [
                ...
                and self.sub_agencies_map[subagency["subtier_agency_code"]] == top_tier_agency["cgac_code"]
            ]
            for subagency in response_data["sub_agencies"]:
                del subagency["subtier_agency_code"]

            response_data["federal_accounts"] = (
                FederalAccount.objects.filter(agency_identifier=top_tier_agency["cgac_code"])
